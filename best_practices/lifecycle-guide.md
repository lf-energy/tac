...
This is designed to be a practical guide for projects to help them move through the [LF Energy project lifecycle stages](/process/lifecycle.html). This guide helps project know what to focus on at each stage and what is needed to advance to the next one.
...
Once your project is approved by the LF Energy TAC, the work begins! Sandbox projects are expected to reach Incubation within one year, and there is lots to do to get the house in order: IP, tooling, governance foundations, and early community signals.

### What to focus on while here

#### Start the OpenSSF Passing badge immediately

The badge has ~65 criteria and takes weeks to complete properly — don't leave it until the last month. Visit [bestpractices.dev](https://bestpractices.dev) and work through each criterion from day one. Common quick wins: adding a CONTRIBUTING file, specifying your license clearly, and setting up basic CI.

Also set up a vulnerability reporting process early. Add a `SECURITY.md` file and a private disclosure channel (e.g. a `security@` email or GitHub private vulnerability reporting). This is a badge requirement and signals maturity to potential adopters. More details on how to set this up are in the [Repository Setup Recommendations](repo.html#security).
...
Guideance for setting these core files up are in the [Repository Setup Recommendations](repo.html).
...
- **OpenSSF Passing badge** — ~65 criteria covering: a public bug tracker, a contributing guide, automated tests, a documented build process, and a security vulnerability disclosure policy. Work through each criterion at [bestpractices.dev](https://bestpractices.dev).
...
- **`README.md` in every repository** — explains what the repo is, why it exists, and how to get started. Follow the [README checklist](repo.html#readme) for guidance.
- **`GOVERNANCE.md`** — documents how technical decisions are made, how commit access is granted, how TSC members are elected or removed, and what happens if the project goes dormant. Use the [LF Energy TSC template](https://github.com/lf-energy/tsc-template/blob/main/GOVERNANCE.md) and [guidelines](repo.html#governance)
- **TSC members registered in LFX Project Control Center** — all current TSC members listed under the project's TSC committee in LFX PCC. [LF Staff](mailto:support@lfenergy.org) can help with setup.
...
> **Governance is what the TAC looks at most.** They want evidence the project can function without its founders. A documented TSC with real decision-making authority signals long-term health far more than code quality alone.
...
The project's governance foundations are in place. Now the focus is to build a community around the project, with the goal of ensuring the project has a life beyond its creators. Things to focus on include growing contributors from multiple organizations, identifying real production users, and building the technical quality needed for Early Adoption.

### What to focus on while you're here

**Work toward the OpenSSF Silver badge**

Silver adds ~55 additional criteria on top of Passing, with stronger requirements around test coverage (statements and branches), static analysis in CI, a documented security review process, and contributor response time standards. Start by reviewing the [Silver criteria](https://bestpractices.dev/en/criteria/1) and categorizing each as "already done," "in progress," or "needs work."
...
> **Production users take time to identify .** A company going from evaluation to production deployment can take 6–18 months. Start those conversations as early as possible in Incubation.
...
- **OpenSSF Silver badge** — adds on top of Passing: documented statement and branch coverage, static analysis in CI, a security review process, architectural documentation, and stricter contributor-response standards. See all criteria at [bestpractices.dev/en/criteria/1](https://bestpractices.dev/en/criteria/1).
...
Gold adds ~15 criteria on top of Silver, including: a documented bus factor of at least 2, a two-person review requirement for code changes, and a documented security assurance case. Review all criteria at [bestpractices.dev/en/criteria/2](https://bestpractices.dev/en/criteria/2). The bus factor requirement has governance implications — you need multiple people capable of doing releases across organizations.

This is also the time to consider a security audit. LF Energy and the Linux Foundation have programs to help fund or connect projects with security auditors. Engaging with this early both strengthens the codebase and demonstrates the security maturity enterprise adopters expect.
